...
const Users = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const res = await axios.get(
          `https://chat-app-sentimentai.onrender.com/api/user/fetchUsers?search=${search}`,
          {
            headers: {
  ...
  //     try {
  //         const token = JSON.parse(localStorage.getItem('authToken'))
  //         const res = await axios.get(`https://chat-app-sentimentai.onrender.com/api/chat/fetchChat/${userId}`,{
  //             headers: {
  //                 Authorization: `Bearer ${token}`
  //             }
  //         })
  ...
  return (
    <div
      className={`${
        darkMode && "dark-theme"
      } flex flex-col h-full flex-1 md:flex-[0.7] bg-slate-100 px-4`}
    >
      <div className="flex flex-col gap-1 mt-2 w-full h-full">
        <div
          className={`${
            darkMode && "dark-primary"
          } flex items-center w-full bg-white mt-3 py-2 rounded-xl`}
        >
          ...
          <h1 className="p-3 font-semibold">Add Users</h1>
        </div>
        <div>
        ...
        <div
          className={`${
            darkMode && "dark-primary"
          } bg-white flex-1 mb-3 rounded-xl p-3 pt-0`}
        >
          <div className="flex flex-col overflow-auto mt-1 mb-3">
            {users.length > 0 &&
              users.map((user) => (
                <div
                  onClick={() => navigate(`/app/user/${user._id}`)}
                  key={user._id}
                  className={`cursor-pointer flex items-center py-3 border-b-2 ${
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="h-10 w-10 relative border rounded-full">
                      ...
                      <div
                        className={`${
                          user.isOnline ? "flex" : "hidden"
                        } absolute w-3 h-3 bg-green-500 rounded-full right-0 bottom-0`}
                      ></div>
                    </div>
                    {user.name}
                  </div>
