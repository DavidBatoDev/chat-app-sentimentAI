...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (!user) return;
    fetchUsersChat();
    localStorage.getItem("highlightedConvos") &&
      setHighlightedConvos(
        JSON.parse(localStorage.getItem("highlightedConvos"))
      );
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on("new chat", (chat) => {
      ...
      localStorage.setItem(
        "highlightedConvos",
        JSON.stringify([...highlightedConvos, chat._id])
      );
    ...
    socket.on("delete chat", (chat) => {
      ...
      setConvos((prevConvos) =>
        prevConvos.filter((convo) => convo._id !== chat._id)
      );
      setHighlightedConvos((prevState) =>
        prevState.filter((id) => id !== chat._id)
      );
      localStorage.setItem(
        "highlightedConvos",
        JSON.stringify(highlightedConvos.filter((id) => id !== chatId))
      );
    });
    ...
    socket.on("update message", (message) => {
      if (message.sender._id !== user._id && message.chat._id !== currentChat) {
        ...
        localStorage.setItem(
          "highlightedConvos",
          JSON.stringify([...highlightedConvos, message.chat._id])
        );
      }
    });
  ...
  const handleConversationClick = (convoId) => {
    ...
    localStorage.setItem(
      "highlightedConvos",
      JSON.stringify(highlightedConvos.filter((id) => id !== convoId))
    );
  ...
  return (
    <div
      className={`hidden sm:block h-full bg-slate-300 flex-[0.3] ${
    ...
    >
      ...
      <nav
        ...
        } flex justify-between items-center my-5 mx-2 rounded-xl p-3 bg-white`}
      >
        <div
        ...
        >
          ...
          <p className="hidden font-semibold lg:flex cursor-pointer">
            {user.name}
          </p>
        </div>
