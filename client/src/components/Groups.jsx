...
const Groups = () => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      const res = await axios.get(
        `https://chat-app-sentimentai.onrender.com/api/user/fetchGroups?search=${search}`,
        {
          headers: {
  ...
  const handleJoinGroup = async (groupId) => {
    try {
      ...
      const token = JSON.parse(localStorage.getItem("authToken") || null);
      const res = await axios.get(
        `https://chat-app-sentimentai.onrender.com/api/chat/joinGroup/${groupId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    >
      ...
      <div className="flex flex-col gap-1 mt-2 w-full h-full">
        ...
        >
          ...
          <h1 className="p-3 font-semibold">Join Public Groups</h1>
        </div>
        <div>
          ...
          <div className="flex flex-col overflow-auto mt-1 mb-3">
            {groups.map((group) => (
              <div
                ...
                className={`flex items-center py-3 border-b-2 cursor-pointer ${
