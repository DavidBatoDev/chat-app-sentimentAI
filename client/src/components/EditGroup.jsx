...
const EditGroup = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const res = await axios.get(
          `https://chat-app-sentimentai.onrender.com/api/user/fetchRelatedUsers`,
          {
            headers: {
  ...
  const fetchGroupInfo = async () => {
    try {
      ...
      const res = await axios.get(
        `https://chat-app-sentimentai.onrender.com/api/chat/fetchChatById/${chatId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleAddUser = () => {
    const addedUsers = fetchedUsers.filter((user) =>
      usersToBeAdded.includes(user._id)
    );
    setFormBody({
      ...formBody,
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `https://chat-app-sentimentai.onrender.com/api/chat/update-group/${chatId}`,
        body,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const nameOfTheUserRemoved = res.data.usersThatIsRemoved
        .map((user) => {
          return fetchedUsers.find((u) => u._id === user).name;
        })
        .join(", ");
      const nameOfTheUserAdded = res.data.usersThatIsAdded
        .map((user) => {
          return fetchedUsers.find((u) => u._id === user).name;
        })
        .join(", ");
      ...
      const resForSendMessage = await axios.post(
        `https://chat-app-sentimentai.onrender.com/api/message`,
        {
          content: message,
          chatId: chatId,
        },
  ...
  return (
    <div
      className={`flex flex-col h-full md:flex-[0.7] flex-1 px-4 ${
        darkMode ? "dark-theme" : "bg-slate-100"
      }`}
    >
      <Backdrop
      ...
      <form
        ...
        className={`h-full mb-3 p-4 mt-5 rounded-xl flex flex-col ${
          darkMode ? "bg-gray-800" : "bg-white"
        } shadow overflow-y-scroll custom-scrollbar`}
      >
        ...
        <div className="flex flex-col items-center w-full gap-5 md:px-2 lg:px-10">
          ...
          <div
            className={`w-full rounded-xl flex flex-col gap-3 p-3 ${
          ...
          <div
            className={`w-full rounded-xl flex flex-col gap-5 p-3 ${
              darkMode ? "bg-gray-700" : "bg-gray-200"
            }`}
          >
            <div>
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              className={`text-lg rounded-full px-10 py-2 w-full ${
      ...
      >
        ...
        >
          <div>
            ...
            {fetchedUsers
              .filter(
                (user) => !groupInfo?.users.find((u) => u._id === user._id)
              )
              .map((user) => (
                <div
                  key={user._id}
          ...
          <div className="flex justify-end gap-4">
            <button
              className={`text-lg rounded-full px-3 py-2 ${
                darkMode ? "bg-gray-600 text-white" : "bg-gray-200 text-black"
              }`}
